...
is het niet alleen belangrijk hoe snel je rijdt <Mute text="(= grootte van de
vector)" />, maar ook waar je naartoe rijdt <Mute text="(= richting en zin van
de vector)" />. We schrijven de snelheidsvector als $\vec{v}$.
...
vergroot de vector $\vec{v}$ <Mute text="(je gaat sneller vooruit)" />.

![De snelheidsvector vergroot](img/snelheid_vergroot.png)

Wanneer je remt, verkleint de vector $\vec{v}$ <Mute text="(je gaat trager vooruit)" />.
...
<Attention title="$\vec{F}_r$ bepaalt de snelheidsverandering">
