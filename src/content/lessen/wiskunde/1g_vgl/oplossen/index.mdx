...
waar er maar één onbekende is <Mute text="(genaamd $x$)" /> en waarbij de
...
In de eerste stap trekken we $b_{links}$ <Mute text="($= -2$)" /> af van het linker- en rechterlid:
$$
\begin{aligned}
\Leftrightarrow -5x - 2 &= 12\\
...
linker- en rechterlid te delen door $a$ <Mute text="($= -5$)" />.
