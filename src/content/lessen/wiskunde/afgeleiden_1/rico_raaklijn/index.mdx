...
een maat voor **hoe steil** de rechte is <Mute text="(herinner je dat een
eerstegraadsfunctie altijd een rechte is)" />.  Je kan de rico van een
eerstegraadsfunctie als volgt uit de grafiek van de functie halen:

1. Ga ergens op de rechte staan, maakt niet uit waar
2. Schuif één stapje <Mute text="(een eenheid)" /> op in de x-richting
...
| 
$$
\frac{\green{f(x\_2)} - \green{f(x\_1)}}{\orange{\orange{x\_2} - \orange{x\_1}}}
$$
 | 
$$
\frac{\green{f(x\_2)} - \green{f(x\_1)}}{\orange{\orange{x\_2} - \orange{x\_1}}}
...
we eigenlijk ook de **rico** van de eerstegraadsfunctie <Mute text="(of de
rechte)" /> die door de punten met coördinaten $(\orange{x\_1},
\green{f(x\_1)})$ en $(\orange{x\_2}, \green{f(x\_2)})$ gaat!
...
te berekenen waarbij we $\Delta x$ <Mute text="(of $x_2 - x_1$)" /> naar $0$
laten gaan:
...
<Attention title="De afgeleide als de rico van een raaklijn">
